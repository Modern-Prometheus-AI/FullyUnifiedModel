...
- The model is now capable of introspective thought using an Emergent Hierachical Synaptic Probe, automatically occurring when learning stagnation or repeated failure at completing a goal. When this happens, the knowledge graph can solve a problem in thought, and self organize to solve the problem, reducing the likelihood that the failure will happen again. Over time this feature is naturally used less and less as it becomes smarter. This cleverly combines a sparse search algorithm that transitions into a dense squareform array when the resolution of the search gets low enough. This allows the model to rapidly "zoom in" and access new or remembered solutions to problems with a high level of granularity, maintaining a subquadratic time complexity in memory and offers nearly the same results as a fully quadratic dense array search implementation.
